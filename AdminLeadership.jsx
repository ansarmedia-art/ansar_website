...
export default function AdminLeadership() {
  ...
  const [formData, setFormData] = useState({ name: '', role: '', qualification: '', imageUrl: '', order: 0, published: true });
  ...
  const resetForm = () => {
    setEditingId(null);
    setFormData({ name: '', role: '', qualification: '', imageUrl: '', order: 0, published: true });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await addDoc(collection(db, 'leadership'), payload);
      }
      resetForm();
    } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="bg-white p-8 rounded-2xl shadow-xl border border-emerald-100 mb-8">
        <h2 className="text-xl font-bold mb-6 text-slate-800">{editingId ? 'Edit Leader' : 'Add New Leader'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100">
            <label className="flex items-center gap-2 cursor-pointer font-medium text-slate-700"><input name="published" type="checkbox" checked={formData.published} onChange={handleChange} className="w-5 h-5 text-emerald-600 rounded" />Published</label>
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 bg-emerald-600 text-white font-bold rounded-lg hover:bg-emerald-700 transition-colors disabled:opacity-50">{isSubmitting ? 'Saving...' : (editingId ? 'Update Leader' : 'Save Leader')}</button>
            </div>
          </div>
        </form>
      </div>
      ...
      <div className="space-y-4">
        <h3 className="font-bold text-lg text-slate-800 mb-4">Current Leadership</h3>
